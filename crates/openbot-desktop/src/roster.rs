...
//! contract.
//!
//! So this asks `openbot bot ls --json`, which exists for exactly this and is
//! pinned by `openbot-cli/tests/cli.rs`. One subprocess per refresh is
...
pub async fn list(openbot: &Path, home: &Path, hidden: bool) -> anyhow::Result<Vec<Entry>> {
    ...
    serde_json::from_str(text.trim()).map_err(|e| {
        anyhow::anyhow!("`openbot bot ls --json` answered something else ({e}): {text}")
    })
}
...
pub async fn delete(openbot: &Path, home: &Path, bot: &str) -> anyhow::Result<()> {
    let mut cmd = tokio::process::Command::new(openbot);
    cmd.arg("bot")
        .arg("rm")
        .arg(bot)
...
/// anything it has scheduled, which `openbot bot hide` warns about explicitly.
...
/// `openbot bot set --rename` intentionally keeps the id, so after one rename
...
/// visible in it. `openbot group log` reads that thread back.
#[derive(Clone, Debug, Default, Deserialize, PartialEq, Eq, Serialize)]
pub struct Group {
    pub id: String,
    pub name: String,
...
/// # Errors
/// As [`list`].
pub async fn groups(openbot: &Path, home: &Path) -> anyhow::Result<Vec<Group>> {
    let out = run(openbot, home, &["group", "ls", "--json"]).await?;
...
pub async fn group_log(
    ...
    let out = run(openbot, home, &["group", "log", name, "--json"]).await?;
    serde_json::from_str(out.trim())
        .map_err(|e| anyhow::anyhow!("`openbot group log --json` answered something else: {e}"))
}
...
pub async fn search(openbot: &Path, home: &Path, query: &str) -> anyhow::Result<Vec<Hit>> {
    let out = run(openbot, home, &["search", query, "--json"]).await?;
    serde_json::from_str(out.trim())
        .map_err(|e| anyhow::anyhow!("`openbot search --json` answered something else: {e}"))
}

async fn run(openbot: &Path, home: &Path, args: &[&str]) -> anyhow::Result<String> {
    let mut cmd = tokio::process::Command::new(openbot);
    cmd.args(args)
        .arg("--home")
        .arg(home)
...
mod tests {
    ...
    /// The shape is a contract with `openbot bot ls --json`, and the fields the
