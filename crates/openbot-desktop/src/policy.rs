...
//! module and `openbot permission` are the write side of that store.
//!
...
/// One rule, as `openbot permission ls --json` prints it.
#[derive(Clone, Debug, Deserialize, PartialEq, Eq, Serialize)]
pub struct Rule {
    pub action: Action,
...
pub async fn list(openbot: &Path, home: &Path) -> anyhow::Result<Vec<Rule>> {
    ...
    serde_json::from_str(text.trim())
        .map_err(|e| anyhow::anyhow!("`openbot permission ls --json` answered something else: {e}"))
}
...
pub async fn add(openbot: &Path, home: &Path, rule: &Rule) -> anyhow::Result<()> {
    let mut cmd = base(openbot, home);
    cmd.arg("add")
        .arg("--action")
...
pub async fn remove(openbot: &Path, home: &Path, number: usize) -> anyhow::Result<()> {
    let mut cmd = base(openbot, home);
    cmd.arg("rm").arg(number.to_string());
    run(cmd, openbot).await
}

async fn run(mut cmd: tokio::process::Command, openbot: &Path) -> anyhow::Result<()> {
    let out = cmd
        .output()
        .await
        .map_err(|e| anyhow::anyhow!("could not run {}: {e}", openbot.display()))?;
...
mod tests {
    ...
    /// The shape is a contract with `openbot permission ls --json`, which
    /// serialises openbotd's own `Rule`. Either end moving alone is the bug.
    #[test]
