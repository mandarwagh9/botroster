...
pub async fn put(openbot: &Path, hub: &str, file: &Path) -> anyhow::Result<String> {
    let out = base(openbot, hub)
        .arg(file)
        .arg("--json")
        .output()
        .await
    ...
    let v: serde_json::Value = serde_json::from_slice(&out.stdout)
        .map_err(|e| anyhow::anyhow!("`openbot attach` answered something else: {e}"))?;
    v.get("path")
        .and_then(|p| p.as_str())
        .map(str::to_owned)
        .ok_or_else(|| anyhow::anyhow!("`openbot attach` did not say where it landed"))
}

fn base(openbot: &Path, hub: &str) -> tokio::process::Command {
    let mut cmd = tokio::process::Command::new(openbot);
    // Resolved through the hub so the destination is the workspace the guest
    // is actually serving. A home-derived path is correct only when `openbot up`
