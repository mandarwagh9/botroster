...
//! instead of typing any of those into chat. `openbot watch` serves the live
...
//! (`openbot-cli/tests/viewer.rs`) covering its refusals.
...
/// A running `openbot watch`, and where it is serving.
///
...
pub async fn open(openbot: &Path, hub: &str) -> anyhow::Result<Viewer> {
    ...
    let mut cmd = std::process::Command::new(openbot);
    cmd.arg("watch")
        .arg("--hub")
        .arg(hub)
        // 0 picks a free one. A fixed port would collide with a `openbot watch`
        // the person already has open in a terminal.
        .arg("--port")
        .arg("0")
        .env("NO_COLOR", "1")
        // As in `hub::reach`: `openbot watch` declares no home argument, so
        // there is no `OPENBOT_HOME` to scrub. Held by
    ...
    while Instant::now() < deadline {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            ...
            return Err(anyhow::anyhow!(
                "`openbot watch` exited early ({status}): {}",
                said.trim()
            ));
        }
    ...
    let _ = child.kill();
    Err(anyhow::anyhow!(
        "`openbot watch` never said where it was listening"
    ))
}

/// The address `openbot watch` announces, if it has announced one yet.
///
...
mod tests {
    ...
    fn the_address_is_read_out_of_the_banner() {
        let banner = "watching openbot-workspace\n\n  \
    ...
    fn a_banner_that_has_not_arrived_yet_is_not_an_address() {
        ...
        assert_eq!(url_in("watching openbot-workspace\n"), None);
    }
