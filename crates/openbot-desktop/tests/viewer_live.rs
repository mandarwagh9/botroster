...
//! by taking control of the computer rather than typing it into chat. openbot
//! enforces that lock in the hub, and `openbot-cli/tests/viewer.rs` proves the
...
fn agent_says(hub: &str) -> Result<String, String> {
    let out = std::process::Command::new(common::up::openbot())
        ...
        .output()
        .expect("could not run openbot call");
...
async fn the_window_can_open_the_computer_and_lock_the_bot_out_of_it() {
    ...
    let mut view = viewer::open(&common::up::openbot(), &up.hub)
        .await
...
async fn dropping_the_viewer_stops_it_serving() {
    let up = Up::start().expect("openbot up");
    let view = viewer::open(&common::up::openbot(), &up.hub)
        .await
...
async fn a_viewer_that_cannot_reach_a_hub_says_so() {
    let err = viewer::open(&common::up::openbot(), "ws://127.0.0.1:1/v1/tools")
        .await
