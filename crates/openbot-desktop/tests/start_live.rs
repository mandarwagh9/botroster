...
//! type `openbot up`" is not an application. The window starts one itself, and
...
async fn a_started_computer_is_serving_when_the_call_returns() {
    ...
    let reach = hub::reach(&common::up::openbot(), &hub_url)
        .await
