...
//!
//! `roster::list` and `openbot bot ls --json` are two ends of one wire and each
...
/// Any `openbot` command. `make` prepends `bot`, which is right for the tests
...
async fn the_client_reads_the_roster_the_binary_writes() {
    ...
    let visible = roster::list(&common::up::openbot(), home, false)
        .await
    ...
    // "Show hidden chats".
    let all = roster::list(&common::up::openbot(), home, true)
        .await
...
async fn an_untouched_home_has_an_empty_roster() {
    let home = tempfile::tempdir().expect("a home");
    let bots = roster::list(&common::up::openbot(), home.path(), false)
        .await
...
async fn a_roster_that_cannot_be_read_is_an_error_not_an_empty_list() {
    let home = tempfile::tempdir().expect("a home");
    let missing = home.path().join("there-is-no-openbot-here");
...
/// reason to put Bots in a group. `openbot group log` reads that thread back.
#[tokio::test(flavor = "multi_thread")]
async fn the_client_can_read_a_group_and_the_thread_it_preserves() {
    ...
    let groups = roster::groups(&openbot, home).await.expect("groups");
    ...
    let thread = roster::group_log(&openbot, home, "Website Launch")
        .await
        .expect("the thread should be readable");
    assert_eq!(thread, serde_json::json!([]));

    let missing = roster::group_log(&openbot, home, "no-such-group").await;
...
async fn renaming_a_bot_reaches_the_group_and_the_routine_that_hold_it() {
    ...
    let routines = openbot_desktop::settings::routines(&openbot, home)
        .await
        .expect("routines");
