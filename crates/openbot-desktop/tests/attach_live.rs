...
async fn an_attached_file_is_readable_by_the_guest() {
    ...
    let at = openbot_desktop::attach::put(&openbot, &up.hub, &src)
        .await
    ...
    let out = tokio::process::Command::new(&openbot)
        .arg("call")
        .arg("fs.read")
...
async fn two_files_of_one_name_both_survive() {
    ...
    let one = openbot_desktop::attach::put(&openbot, &up.hub, &a.join("report.txt"))
        .await
        .expect("first");
    let two = openbot_desktop::attach::put(&openbot, &up.hub, &b.join("report.txt"))
        .await
        .expect("second");
