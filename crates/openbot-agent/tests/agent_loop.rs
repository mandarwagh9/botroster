...
use openbot_agent::agent::AgentEvent;
use openbot_agent::model::{Content, Role};
use openbot_agent::providers::Scripted;
use openbot_agent::{Agent, AgentConfig, AllowAll, ApprovalHandler, FinishReason, HubClient};
...
struct Rig {
    hub: Arc<HubClient>,
    progress: Option<mpsc::UnboundedReceiver<openbot_proto::frames::ToolCallProgressFrame>>,
...
async fn rig_with(policy: Policy, approver: Arc<dyn ApprovalHandler>) -> anyhow::Result<Rig> {
    let hub_state = Arc::new(openbotd::hub::Hub::with_policy(policy));
    let (listener, addr) = openbotd::server::Server::bind("127.0.0.1:0").await?;
    let server = Arc::new(openbotd::server::Server::new(hub_state));
    ...
    for _ in 0..100 {
        if !client.list_servers().await?.is_empty() {
            tools = client.bind_server("openbot-workspace").await?;
            break;
        }
...
impl Rig {
    async fn run(
        ...
        task: &str,
        max_steps: u32,
    ) -> (openbot_agent::AgentOutcome, Vec<AgentEvent>) {
        let agent = Agent::new(
            model,
            Arc::clone(&self.hub),
            AgentConfig {
    ...
    async fn run_model(
        &mut self,
        model: Arc<dyn openbot_agent::model::Model>,
        task: &str,
        cancel: tokio::sync::watch::Receiver<bool>,
    ) -> openbot_agent::AgentOutcome {
        let agent = Agent::new(
            model,
            Arc::clone(&self.hub),
            AgentConfig {
...
async fn a_provider_having_a_bad_minute_is_marked_worth_repeating() -> anyhow::Result<()> {
    ...
    let model = Arc::new(
        Scripted::builder()
            ...
            .fails(openbot_agent::ModelError::Overloaded(
...
async fn a_rejected_key_is_not_worth_repeating() -> anyhow::Result<()> {
    ...
    let model = Arc::new(
        Scripted::builder()
            .fails(openbot_agent::ModelError::Rejected(
...
impl openbot_agent::model::Model for NeverAnswers {
    async fn turn(
        &self,
        _req: &openbot_agent::model::TurnRequest,
    ) -> Result<openbot_agent::model::TurnResponse, openbot_agent::model::ModelError> {
...
fn unanswered_calls(transcript: &[openbot_agent::model::Message]) -> Vec<String> {
...
impl ApprovalHandler for StopsWhenAsked {
    async fn decide(
        &self,
        _req: &openbot_proto::approval::ApprovalRequestParams,
    ) -> openbot_proto::approval::ApprovalDecision {
        let _ = self.stop.send(true);
        openbot_proto::approval::ApprovalDecision::allow_once()
    }
}
...
#[async_trait::async_trait]
impl openbot_agent::model::Model for CancelsWhileThinking {
    async fn turn(
        &self,
        req: &openbot_agent::model::TurnRequest,
    ) -> Result<openbot_agent::model::TurnResponse, openbot_agent::model::ModelError> {
...
/// A direct message can redirect the current turn. openbot runs one turn at a
...
async fn stopping_beats_redirecting() {
    ...
    let agent = Agent::new(
        Arc::new(Scripted::builder().say("Noted.").build()) as Arc<dyn openbot_agent::model::Model>,
        Arc::clone(&rig.hub),
        AgentConfig {
            system: "test".into(),
            max_steps: 6,
...
async fn two_tool_calls_sharing_an_id_end_the_turn() -> anyhow::Result<()> {
    use openbot_agent::model::{StopReason, ToolUseId, TurnResponse, Usage};
