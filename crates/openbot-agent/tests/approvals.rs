...
use openbot_proto::approval::{ApprovalDecision, ApprovalRequestParams, Decision};
use openbot_proto::frames::ToolDescription;
use openbotd::policy::{Action, Policy, Rule};
use serde_json::json;
use tokio::sync::mpsc;
...
async fn rig(policy: Policy, approver: Arc<dyn ApprovalHandler>) -> anyhow::Result<Rig> {
    rig_timeout(policy, approver, openbotd::hub::DEFAULT_APPROVAL_TIMEOUT).await
}
...
async fn rig_timeout(
    ...
    let hub_state =
        Arc::new(openbotd::hub::Hub::with_policy(policy).with_approval_timeout(approval_timeout));
...
async fn with_no_approver_attached_everything_gated_is_denied() -> anyhow::Result<()> {
    ...
    let hub_state = Arc::new(openbotd::hub::Hub::with_policy(Policy::default()));
    let (listener, addr) = openbotd::server::Server::bind("127.0.0.1:0").await?;
    let server = Arc::new(openbotd::server::Server::new(hub_state));
    tokio::spawn(Arc::clone(&server).serve(listener));
...
/// guaranteed to fail the same way. Left unhandled, a nightly `openbot routine
...
async fn with_no_approver_the_run_stops_at_the_first_gate() -> anyhow::Result<()> {
    ...
    let script = openbot_agent::providers::Scripted::builder()
    ...
    match &outcome.reason {
        openbot_agent::FinishReason::NothingApproved { message, tool } => {
