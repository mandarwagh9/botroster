...
pub fn default_system_prompt() -> String {
    "You are a openbot Bot: a persistent teammate with your own cloud computer.\n\
...
/// current turn. openbot runs one turn at a time per session, and the answer is
...
impl Agent {
    ...
    pub async fn run(
        ...
        mut progress: mpsc::UnboundedReceiver<openbot_proto::frames::ToolCallProgressFrame>,
            ...
            for (id, name, args) in uses {
                ...
                let (ok, output, rendered) = match outcome {
                    ...
                    Err(HubError::Rpc { code, message })
                        if code == openbot_proto::codes::TAKEN_OVER =>
                    {
                        return self.finish(
                    ...
                    Err(HubError::Rpc { code, message })
                        if code == openbot_proto::codes::APPROVAL_DENIED
