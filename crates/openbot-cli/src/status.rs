//! `openbot status`: one screen that answers "why is nothing happening?"
//!
...
pub struct Status {
    ...
    /// belong to.
    ///
    /// A computer can be running and not be on this volume: `openbot up
    ...
    /// rather than believing a file: the same check `openbot computer status`
    /// prints as "attached".
...
pub async fn gather(hub_url: &str, home: &Path, opts: &crate::config::ModelOverrides) -> Status {
    ...
    let hub = match tokio::time::timeout(
        ...
        openbot_agent::HubClient::connect(hub_url),
    )
    .await
    {
    ...
    if let Ok(store) = openbot_bots::BotStore::open(home) {
...
fn newest_snapshot(home: &Path) -> Option<(usize, chrono::Duration)> {
    let store = openbot_store::Store::open(home).ok()?;
...
fn volume_attached(home: &Path) -> bool {
    let Ok(store) = openbot_store::Store::open(home) else {
        return false;
    };
...
/// stops being a status screen. `openbot routine show` has the whole thing.
...
{}",
    ...
    match &s.hub {
        Ok(servers) if servers.is_empty() => row(
            ...
            format!(
                "{}  {}",
                s.hub_url,
                st.yellow("up, but no computer is attached — is openbot-guest running?")
            ),
        ),
        Ok(servers) => row(
            &mut out,
    ...
    match s.snapshots {
        ...
        Some((n, age)) => {
            ...
            let value = if s.hub.is_ok() && !s.volume_attached {
                ...
                st.yellow(&format!("{n}, newest {ago} — is `openbot up` running?"))
            } else {
                format!("{n}, newest {ago}")
            };
    ...
    if !s.due_now.is_empty() {
        // Due now means the next `openbot routine tick` will run it. Its own
        ...
        row(
            ...
            format!(
                "{}  {}",
                s.due_now.join(", "),
                st.dim("run `openbot routine tick`")
            ),
        );
    }
...
mod tests {
    ...
    ///
    /// `openbot up --workspace <dir>` puts the guest on a plain directory that
...
{out}"
    ...
    fn a_healthy_account_reads_plainly() {
        ...
        assert!(out.contains("openbot-workspace"));
    ...
    fn a_routine_failing_every_night_is_on_the_screen() {
        ...
        s.failing = vec![(
            ...
            "[-32003] no tool server registered as `openbot-workspace`".into(),
        )];
        let out = render(&s, Style::plain());
...
{}",
    ...
    fn a_hub_with_no_computer_names_the_missing_piece() {
        ...
        assert!(out.contains("openbot-guest"), "{out}");
    }
