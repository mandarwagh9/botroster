...
pub fn nothing_approved_advice(tool: &str) -> &'static str {
    if tool == "secret.request" {
        ...
        "store it first with `openbot secret set <name>`"
    } else {
...
mod advice_tests {
    ...
    fn a_credential_refusal_is_not_told_to_try_approve_auto() {
        ...
        assert!(said.contains("openbot secret set"), "{said}");
    ...
    fn a_declined_turn_is_named_in_the_summary() {
        let (mark, said) = summary_of(Style::plain(), &openbot_agent::FinishReason::Declined);
        ...
        let (ok, done) = summary_of(Style::plain(), &openbot_agent::FinishReason::Completed);
        assert_eq!(ok, "✓");
