//! `openbot acp`: OPENBOT as an Agent Client Protocol agent, over stdio.
//!
...
//! session; OPENBOT's unit of memory is a named Bot. Binding them means one
...
/// What a live ACP session is attached to on OPENBOT's side.
///
...
enum Target {
    Bot(openbot_bots::BotId),
    Group(openbot_bots::BotId),
}
...
struct Session {
    ...
    ///
    /// A direct message can redirect the current turn. OPENBOT keeps one turn
...
struct Running {
    redirects: openbot_agent::agent::Redirects,
...
fn claim_turn(sessions: &Sessions, sid: &str) -> Result<Claim, String> {
    ...
    s.cancel = Some(stop);
    let redirects = openbot_agent::agent::Redirects::new();
...
/// Everything the person sent, as one string, including what OPENBOT could
/// not read.
///
/// ACP sends a prompt as content blocks and the agent takes a task, so the
/// blocks have to become text. Two kinds are not OPENBOT's to decline:
///
/// Resource links are baseline. The schema says "All agents MUST support
/// resource links in prompts", and no capability declines them. OPENBOT cannot
...
/// gated behind capabilities OPENBOT advertises as `false`, so a well-behaved
...
fn prompt_text(blocks: &[v1::ContentBlock]) -> Result<Prompt, String> {
    ...
    if out.is_empty() {
        return Err(if unread.is_empty() {
            ...
            format!(
                "the prompt held no text openbot can read — only {}",
                counted(&unread)
            )
        });
    }
    let note = if unread.is_empty() {
        ...
        let note = format!(
            "[openbot reads text and links. It could not read {}, so anything \
...
impl openbot_agent::ApprovalHandler for AcpApprover {
        ...
        Ok(res) => {
            match res.outcome {
                v1::RequestPermissionOutcome::Selected(s) => super::decision(&s.option_id.0)
                    .unwrap_or_else(|| {
                        ...
                        ApprovalDecision::deny().with_note(
                            "the ACP client answered with an option openbot did not offer",
                        )
                    }),
                ...
                _ => ApprovalDecision::deny()
                    .with_note("an approval outcome openbot does not understand"),
            }
        }
...
/// OPENBOT never offered, a missing `_meta`, a `_meta` holding something that
    ...
    agent_client_protocol::Agent
            ...
            async move |req: v1::InitializeRequest, responder, _conn| {
                responder.respond(
                    v1::InitializeResponse::new(req.protocol_version)
                        ...
                        // OPENBOT's to never call; see SPEC §9.
                        //
                        // `load_session` is the exception, and OPENBOT needs it
        ...
        .on_receive_request(
            async move |req: v1::NewSessionRequest, responder, _conn| {
                ...
                let id = format!("openbot-{}", uuid::Uuid::new_v4());
        ...
        .on_receive_request(
            async move |req: v1::PromptRequest, responder, conn| {
                ...
                // redirect the current turn; OPENBOT keeps one turn per
...
fn existing_or_new(
    bots: &openbot_bots::BotStore,
    name: &str,
    cwd: &Path,
) -> Result<openbot_bots::Bot, String> {
    ...
    // a sidebar and tell `openbot bot ls` nothing; a field repeating its
...
mod tests {
    ...
    fn started(tool: &str, call: &str) -> AgentEvent {
        AgentEvent::ToolCallStarted {
            step: 1,
            id: openbot_agent::ToolUseId::new("u1"),
    ...
    fn one_session() -> (Sessions, String) {
        let sessions: Sessions = Arc::new(Mutex::new(HashMap::new()));
        let sid = "openbot-test".to_owned();
        sessions.lock().unwrap().insert(
            sid.clone(),
            Session {
                target: Target::Bot(openbot_bots::BotId("b1".to_owned())),
                running: None,
                cancel: None,
            },
        );
    ...
    fn a_conversation_cannot_be_loaded_over_a_running_turn() {
        ...
        let bot = Target::Bot(openbot_bots::BotId("someone-else".to_owned()));
    ...
    fn an_idle_session_can_be_rebound_to_a_conversation() {
        let (sessions, sid) = one_session();
        let bot = Target::Bot(openbot_bots::BotId("elsewhere".to_owned()));
    ...
    fn every_kind() -> Vec<AgentEvent> {
        ...
        vec![
            ...
            AgentEvent::ToolProgress {
                call_id: openbot_proto::ToolCallId::new("c1"),
                payload: serde_json::json!({ "stage": "running echo openbot-ok" }),
            },
            AgentEvent::ToolCallFinished {
                id: openbot_agent::ToolUseId::new("u1"),
                call_id: openbot_proto::ToolCallId::new("c1"),
            ...
            AgentEvent::Finished {
                reason: openbot_agent::FinishReason::Completed,
                steps: 1,
                usage: Usage::default(),
                text: "done".into(),
            },
    ...
    fn progress_reaches_the_client_as_the_stage_it_is_at() {
        ...
        assert_eq!(t.text, "running echo openbot-ok");
    }

    /// A payload nobody has agreed the shape of has nothing a client could put
    /// in front of a person. Inventing one would show a shape OPENBOT made up.
    #[test]
...
mod secret_ask_tests {
    ...
    fn only_a_real_value_under_the_agreed_key_is_a_credential() {
        ...
        let cases: Vec<(&str, v1::RequestPermissionResponse)> = vec![
            (
                "an option openbot never offered",
    ...
    fn the_request_carries_the_name_and_the_reason() {
        ...
        assert!(SECRET_META.starts_with("openbot/"), "{SECRET_META}");
    }
}
