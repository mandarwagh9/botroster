//! The reference ACP SDK driving `openbot acp` as a real client would.
//!
...
async fn the_sdk_client_drives_the_shipped_binary_through_a_whole_turn() {
    ...
    let agent = AcpAgent::new(
        AcpAgentConfig::new(env!("CARGO_BIN_EXE_openbot"))
            .arg("acp")
            .arg("--demo")
            .arg("--home")
            .arg(home.path().to_str().expect("a utf-8 home path"))
            .env("OPENBOT_HUB_URL", &up.hub)
            .env("NO_COLOR", "1"),
    );
    ...
    assert_eq!(
        protocol,
        ProtocolVersion::V1,
        "openbot must answer in the version the client spoke"
    );
    assert!(
        advertises_load,
        "openbot stopped offering session loading, so a reconnecting client would show an empty transcript beside a Bot that remembers everything"
    );
    assert!(
        session_id.starts_with("openbot-"),
