...
//! notice if the chain from `openbot connector add` to a tool call has come
...
//! real `openbot up`, calls the tool through the hub, and asks the mock remote
//! what it received.
...
fn a_connector_added_from_the_cli_is_callable_and_keeps_its_token() {
    ...
    let mut up = Command::new(OPENBOT)
        .arg("up")
    ...
    let called = Command::new(OPENBOT)
        .args([
            "call",
    ...
    assert!(
        ...
        "the credential was printed by `openbot up`"
    );
}
