...
//!
//! Starts the real `openbot up` on an ephemeral port, reads the port out of its
...
fn up_serves_a_computer_that_the_other_commands_can_reach() {
    ...
    assert!(
        servers.contains("openbot-workspace"),
...
fn the_quickstart_demo_is_the_one_in_the_readme() {
    ...
    // Character for character what `openbot up` prints as the next command,
...
fn a_bot_handoff_made_by_a_tool_call_reaches_the_inbox() {
    ...
    // it through `openbot call` exercises that path exactly as a model would.
    let mk = |name: &str| {
        Command::new(OPENBOT)
            .args(["bot", "--home"])
            .arg(&home)
    ...
    std::thread::spawn(move || {
        for stream in listener.incoming().flatten() {
            std::thread::spawn(move || {
                ...
                let body = b"<html><head><title>openbot cli test</title></head>\
...
///
/// `openbot up` computes its browser profile from the workspace, and Chrome
...
fn the_browser_works_when_up_is_run_from_a_relative_directory() {
    if openbot_guest::browser::find_browser().is_none() {
        assert!(
    ...
    // gets by typing `openbot up` in a project folder.
    ...
    assert!(
        out.status.success(),
        "browser.open failed under `openbot up`: {stderr}{stdout}"
    );
    assert!(
        stdout.contains("openbot cli test"),
...
fn the_computers_files_are_on_the_durable_volume() {
    ...
    // exists to deliver that. If `openbot up` worked in a plain directory
    // while `snapshot`, `restore` and `prune` operated on
    // `<home>/volumes/<id>/current`, the two would never meet and `openbot
    ...
    let restore = Command::new(OPENBOT)
    ...
    fn pump_until(&mut self, id: i64) -> (serde_json::Value, Vec<serde_json::Value>) {
        ...
        loop {
            ...
            match msg["method"].as_str() {
                ...
                // clicking "Allow once" would, using the id OPENBOT itself
...
fn an_acp_client_can_drive_a_whole_turn() {
    ...
    // tests; reaching it from here needs the richer script `openbot run
    // --demo` uses.
    assert!(
...
///
/// `openbot group post` shares `run_task` with every other turn, and the
...
/// `openbot-guest` may never reach `openbotd` (`crates/openbot-guest/tests/
...
fn every_tool_the_computer_offers_has_a_rule_of_its_own() {
    ...
    let policy = openbotd::policy::Policy::default();
    ...
    for line in listed.lines() {
        ...
        if let openbotd::policy::Verdict::Ask(why) = policy.evaluate(tool, &serde_json::json!({})) {
...
/// guest, `bot` from the hub, `openbot` held back), and this is what keeps it
...
/// `openbot-guest` may never reach `openbotd`, so the catalogue is the only
...
fn every_namespace_served_is_one_a_connector_cannot_claim() {
    ...
    let unreserved: Vec<&String> = prefixes
        .iter()
        .filter(|p| !openbotd::connector::RESERVED.contains(&p.as_str()))
        .collect();
    assert!(
        unreserved.is_empty(),
