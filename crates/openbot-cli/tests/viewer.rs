...
//!
//! `openbot watch` opens an HTTP port that can drive a computer signed into
...
fn start() -> Viewer {
    ...
    let banner = wait_for(&up_log, "ws://").expect("`openbot up` never announced a hub");
    ...
    let watch = Proc(
        Command::new(OPENBOT)
            ...
            .stderr(Stdio::null())
            .spawn()
            .expect("openbot watch"),
    );

    let text = wait_for(&watch_log, "http://").expect("`openbot watch` never announced a url");
...
fn the_page_needs_the_key_and_so_does_every_route() {
    ...
    assert!(page.contains("openbot"), "that is not the viewer page");
    assert!(
...
fn taking_control_through_the_viewer_locks_the_agent_out() {
    ...
    // that `/takeover` answers 200; openbotd's `takeover.rs` checks that a
