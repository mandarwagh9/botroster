//! `openbotd`, the OPENBOT control plane.
//!
...
use clap::Parser;
use openbot_proto::PROTOCOL_VERSION;

use openbotd::server::Server;

#[derive(Parser, Debug)]
#[command(name = "openbotd", about = "openbot control plane")]
struct Args {
    ...
    #[arg(long, env = "OPENBOT_OIDC_ISSUER")]
    ...
    /// any other action.
    #[arg(long, env = "OPENBOT_HOME", default_value = "./openbot-data")]
    ...
    #[arg(long, env = "OPENBOT_HOOK_FAILURE", default_value = "closed")]
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "openbotd=info".into()),
        )
        .init();
    ...
    let booted =
        openbotd::boot::hub_from_home(&args.home, openbotd::policy::Policy::default()).await?;
    let hub = booted.hub;
