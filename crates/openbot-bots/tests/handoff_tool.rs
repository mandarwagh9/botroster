...
use openbot_agent::{Agent, AgentConfig, AllowAll, ApprovalHandler, DenyAll, HubClient};
use openbot_bots::BotStore;
use openbot_proto::frames::ToolDescription;
use openbotd::bot_tools::BotTools;
...
async fn rig(policy: Policy) -> anyhow::Result<Rig> {
    ...
    let hub = Arc::new(
        openbotd::hub::Hub::with_policy(policy)
...
async fn handing_off_to_someone_who_does_not_exist_fails_the_call() -> anyhow::Result<()> {
    ...
    let failed = outcome
        ...
        .any(|c| matches!(c, openbot_agent::Content::ToolResult { is_error: true, .. }));
...
async fn bot_list_does_not_offer_the_caller_itself() -> anyhow::Result<()> {
    ...
    let results: String = outcome
        ...
        .filter_map(|c| match c {
            openbot_agent::Content::ToolResult { content, .. } => Some(content.clone()),
            _ => None,
        })
        .collect();
    assert!(
