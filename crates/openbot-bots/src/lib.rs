...
impl BotStore {
    ...
    pub fn append(&self, id: &BotId, messages: &[Message]) -> Result<()> {
        ...
        let messages = match self.history(id, Some(1))?.first() {
            Some(last) if last.role == openbot_agent::Role::User && *last == messages[0] => {
                &messages[1..]
            }
            _ => messages,
        };
...
mod tests {
    ...
    /// composer in `openbot-app/ui/main.js` inserts the id, which is also the
...
impl BotStore {
    ...
    ///
    /// Counts newlines rather than parsing. `openbot group ls` calls this once
...
mod group_tests {
    ...
    /// after a crash.
    ///
    /// `openbot group ls` calls it once per group, so parsing would deserialise
    ...
    /// `openbot-app/ui/main.js` inserts `el.dataset.mention` for this reason.
    #[test]
