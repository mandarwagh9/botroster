//! openbot guest daemon: runs inside the computer and serves tools to the hub.
//!
...
/// yield the relative `.openbot-browser`, which the browser resolves against the
...
pub fn profile_beside(workspace: &std::path::Path) -> std::path::PathBuf {
    ...
    root.parent().map_or_else(
        || std::env::temp_dir().join("openbot-browser"),
        |parent| parent.join(".openbot-browser"),
    )
}
...
/// `openbot computer force-detach` exists.
...
mod profile_tests {
    ...
    fn a_profile_sits_beside_the_workspace_and_stays_put() {
        ...
        assert_eq!(a.file_name().unwrap(), ".openbot-browser");
        assert_eq!(
            a.parent(),
