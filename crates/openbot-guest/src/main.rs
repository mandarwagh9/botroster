//! `openbot-guest`: the guest tool server.
//!
...
use clap::Parser;
use openbot_guest::{tools, Context, GuestConfig, Workspace};
use openbot_proto::{ConnectionKind, PROTOCOL_VERSION};
...
#[derive(Parser, Debug)]
#[command(name = "openbot-guest", about = "openbot guest tool server")]
struct Args {
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "openbot_guest=info".into()),
        )
        .init();
    ...
    let (root, profile, volume_note) = match &args.store {
        Some(store_root) => {
            let store = openbot_store::Store::open(store_root)?;
            let id = args
                .volume
                .clone()
            ...
            _attached = Some(vol.attach().map_err(|e| {
                anyhow::anyhow!(
                    "cannot attach volume `{id}`: {e}\n\
                     If no guest is running, clear it with `openbot computer force-detach`."
                )
            })?);
        ...
        None => {
            ...
            let profile = openbot_guest::profile_beside(&ws);
    ...
    let result = tokio::select! {
        r = openbot_guest::run_supervised(cfg, Arc::clone(&ws)) => r,
        _ = openbot_guest::stop_signal() => {
